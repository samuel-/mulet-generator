const lines = [
    [
        'a-dada sur mon mulet', 
        'tristes mèches musardent...', 
        'chauve qui peut !',
        'amulez le feu !',
    ],
    [
        '1 mulet de perdu, 10 paires de mules de retrouvées',
        'ainsi vivent la cigale et le mulet',
        'une certaine amulette',
    ],
    [
        'la suspension hydraulique dérape...',
        'même sans pendentif',
        'j\'entends le bruit des sans pou ni loi', 
    ],
    [
        'en sautant du trottoir je me suis cassé le mulet',
        [
            'un mulet n\'est pas qu\'une mèche et bien plus qu\'une sauce escabèche',
            'ici j\'hume cette mèche, là cette sauce escabèche',
        ],
        [
            'nous en avions le mulet tout hérissé',
            'ce mulet me hérisse le poil',
        ],
        'les mulets muent lorsque ça ne va plus',
    ],
];
